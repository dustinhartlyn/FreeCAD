...
DiagnosisCache System::saveDiagnosis() const
{
    ...
    // Copy tag vectors directly (they're just vector<int>)
    ...
    // Save redundant set as clist indices
    for (int i = 0; i < int(clist.size()); i++) {
        if (redundant.count(clist[i])) {
            cache.redundantIndices.push_back(i);
        }
    }

    // Save dependent parameter groups as plist indices
    for (const auto& group : pDependentParametersGroups) {
        ...
        cache.dependentParamGroupsIndices.push_back(indices);
    }

    // Save flat dependent parameter list as plist indices
    for (double* p : pDependentParameters) {
        for (int i = 0; i < int(plist.size()); i++) {
            if (plist[i] == p) {
                cache.dependentParamIndices.push_back(i);
                break;
            }
        }
    }
...
{
    // Restore tag vectors directly (no re-derivation needed)
    ...
    // Restore redundant set using new clist pointers
    redundant.clear();
    for (int idx : cache.redundantIndices) {
        if (idx < int(clist.size())) {
            redundant.insert(clist[idx]);
        }
    }

    // Restore dofs and emptyDiagnoseMatrix
    dofs = cache.dofs;
    emptyDiagnoseMatrix = cache.emptyDiagnoseMatrix;

    // Restore dependent parameter groups using new plist pointers
    pDependentParametersGroups.clear();
    for (const auto& indices : cache.dependentParamGroupsIndices) {
        std::vector<double*> group;
        for (int idx : indices) {
            if (idx < int(plist.size())) {
                group.push_back(plist[idx]);
            }
        }
        pDependentParametersGroups.push_back(group);
    }

    // Restore flat dependent parameter list using new plist pointers
    pDependentParameters.clear();
    ...
    // Set hasDiagnosis so initSolution() skips diagnose()
    hasDiagnosis = true;

    // INVARIANT: clist ordering is identical before and after rebuild because:
    // 1. addConstraints() iterates ConstraintList in order
    // 2. The fingerprint includes a per-element geometry-type sequence
    // 3. The effective-constraint skip predicate is deterministic
    // Therefore clist[i] after rebuild corresponds to the same logical constraint
    // as clist[i] before clear(), making index-based remapping safe.
}
