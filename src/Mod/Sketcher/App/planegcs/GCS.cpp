...
int System::diagnose(Algorithm alg)
{
    ...
    const bool forceMonolithic = !useComponentDiagnose;
    ...
    if (components.size() > 1 && !forceMonolithic) {
        return diagnoseComponentwise(alg, pdiagnoselist, tagmultiplicity, components);
    }
    return diagnoseMonolithic(alg, pdiagnoselist, tagmultiplicity, jacobianRows);
}
